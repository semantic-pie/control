...
WORKDIR=$(pwd)
KB_PATHS='sc-memory/kb/repo.path'
KB_DIR='sc-memory/kb'
mkdir -p  $KB_DIR
touch $KB_PATHS # create if not exist
GIT_KB_PATHS='sc-memory/git.repo.path'
...
Description:
    control - script that allows you to install knowledge bases and manage pie-tunes services.
